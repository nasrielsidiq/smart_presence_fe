    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                const response = await fetch(`${ENDPOINTS.DIVISIONS}/${id}`, {
                    method: 'GET',
                    headers: {
                ...
                const responseOffices = await fetch(ENDPOINTS.OFFICES, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'ngrok-skip-browser-warning': 'true',
                ...
                const resultOffices = await responseOffices.json();
                if (!response.ok) {
                    const errorMessages = resultOffices.errors.map(error => `${error.path}: ${error.msg}`).join(', ');
                    throw new Error(errorMessages || `HTTP error! status: ${response.status}`);
                }
                setOffices(resultOffices.offices.map(office => ({ value: office.id, label: office.name })));
            ...
            <div id="background" className="p-8 rounded-lg bg-white/30 border-2 border-white w-2/3">
                <form className="bg-white rounded-lg shadow-lg overflow-hidden" onSubmit={handleSubmit}>
                    <div className="px-6 py-3 flex justify-between bg-white items-center shadow-md">
                        <h3 className="text-lg font-bold text-blue pt-2">Update Division</h3>
                        ...
                        <div className="mt-2">
                            <div className="grid grid-cols-2 gap-4 mt-4">
                                <div className="mx-5">
                                    ...
                                    <input type="text" name="name" className="border-2 border-dodgerBlue rounded-lg hover:bg-blue-100 text-dodgerBlue mt-2 px-4 py-2 w-full" value={formData.name} onChange={handleChange} />
                                </div>
                                <div className="mx-5">
                                    ...
                                    <Select
                                        ...
                                        styles={customStyles}
                                        value={offices.find(option => option.value === formData.office_id) || null} // Set value based on selected option
                                    />
                                </div>
                            </div>
                        </div>
