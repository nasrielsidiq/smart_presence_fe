...
const ModalCreate = ({ closeModal }) => {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                setEmployee(result.employees.map(employees => ({ value: employees.id, label: employees.full_name })));
        ...
        setType([
            ...
            { value: 'maternity', label: 'Bersalin' },
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const token = Cookies.get('token');
            const response = await fetch(ENDPOINTS.LEAVE, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + token
            ...
            <div id="background" className="p-8 rounded-lg bg-white/30 border-2 border-white w-2/3">
                <form className="bg-white rounded-lg shadow-lg overflow-hidden" onSubmit={handleSubmit}>
                    <div className="px-6 py-3 flex justify-between bg-white items-center shadow-md">
                        <h3 className="text-lg font-bold text-blue pt-2">Add Leave</h3>
                        ...
                        <div className="mt-2">
                            <div className="grid grid-cols-2 gap-4 mt-4">
                                ...
                                <div className="mx-5">
                                    ...
                                    <input type="date" name="start_date" className="border-2 border-dodgerBlue rounded-lg hover:bg-blue-100 text-dodgerBlue mt-2 px-4 py-2 w-full" value={formData.start_date} onChange={handleChange} />
                                </div>
                                <div className="mx-5">
                                    <label className="block text-gray-700 font-medium">End</label>
                                    <input type="date" name="end_date" className="border-2 border-dodgerBlue rounded-lg hover:bg-blue-100 text-dodgerBlue mt-2 px-4 py-2 w-full" value={formData.end_date} onChange={handleChange} />
                                </div>
                            </div>
                            <div className="mx-5 mt-4">
                                <label className="block text-gray-700 font-medium">Reason</label>
                                <textarea type="text" name="reason" className="border-2 border-dodgerBlue rounded-lg hover:bg-blue-100 text-dodgerBlue mt-2 px-4 py-2 w-full" value={formData.reason} onChange={handleChange} />
                            </div>
                        </div>
                    </div>
