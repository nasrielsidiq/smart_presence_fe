...
const ModalCreate = ({ closeModal }) => {
    const [formData, setFormData] = useState({
        ...
        username: '',
        password: '',
        privilege: 'admin' // Pastikan nilai default sesuai dengan nilai yang diharapkan oleh server
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            formDataToSend.append('privilage', formData.privilege);
            if (imageProfile) {
                    ...
                    <div className="px-6 py-4">
                        ...
                        <div className="mt-2">
                            <div className="flex items-center space-x-4">
                                ...
                                <div>
                                    <input name='image_profile' type="file" className="border-2 border-dodgerBlue focus:border-dodgerBlue rounded-lg px-4 py-2 text-dodgerBlue" onChange={handleChange} />
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4 mt-4">
                                <div className="mx-5">
                                    <label className="block text-gray-700 font-medium">Serial ID</label>
                                    <input type="text" name="serial_id" className="border-2 border-dodgerBlue focus:border-dodgerBlue rounded-lg hover:bg-blue-100 text-dodgerBlue mt-2 px-4 py-2 w-full" value={formData.serial_id} onChange={handleChange} />
                                </div>
                                <div className="mx-5">
                                    <label className="block text-gray-700 font-medium">Username</label>
                                    <input type="text" name="username" className="border-2 border-dodgerBlue focus:border-dodgerBlue rounded-lg hover:bg-blue-100 text-dodgerBlue mt-2 px-4 py-2 w-full" value={formData.username} onChange={handleChange} />
                                </div>
                                <div className="mx-5">
                                    <label className="block text-gray-700 font-medium">Password</label>
                                    <input type="password" name="password" className="border-2 rounded-lg border-dodgerBlue focus:border-dodgerBlue hover:bg-blue-100 text-dodgerBlue mt-2 px-4 py-2 w-full" value={formData.password} onChange={handleChange} />
                                </div>
                                <div className="mx-5">
                                    <label className="block text-gray-700 font-medium">Privilege</label>
                                    <select name="privilege" className="border-2 border-dodgerBlue rounded-lg hover:bg-blue-100 focus:border-dodgerBlue text-dodgerBlue mt-2 px-3 py-2 w-full" value={formData.privilege} onChange={handleChange}>
